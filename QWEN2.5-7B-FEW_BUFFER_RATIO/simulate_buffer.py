...
def load_good_decisions():
    ...
    if GOOD_DECISIONS_DF is None:
        csv_path = '/workspace/QWEN2.5_42_7b_BUFFER/data/gpt-3-noperception-reflection-1-100agents-240months_merge/good_decisions_merged.csv'
...
def get_economic_state(env):
    ...
    def _monthly_gdp_proxy(t: int) -> float:
        ...
        for i in range(env.num_agents):
            ...
            if isinstance(st, dict):
                skill = float(st.get('skill', 0.0) or 0.0)
    ...
    if env.world.timestep < 24 or len(actions) < 24:
        ...
        for t in range(len(actions)):
            employed_t = sum(1 for i in range(env.num_agents) if _get_labor_action(actions[t], str(i)) == 1)
    ...
    # 状态划分：尽量对齐离线 good_decisions_merged.csv 的 macro_state 边界
    if gdp_growth <= -0.02:
        return "Recession", unemployment_rate, gdp_growth, wage_growth
    if gdp_growth >= 0.058:
        return "Boom", unemployment_rate, gdp_growth, wage_growth
    return "Normal", unemployment_rate, gdp_growth, wage_growth
...
- Recent GDP growth: {gdp_growth*100:.1f}% (rolling annual)
