...
path = r"/workspace/QWEN2.5-7B-FEW_BUFFER_RATIO/data/gpt-3-noperception-reflection-1-100agents-240months_4b_few_br_1/dense_log.pkl"
...
# 约束不满足率：年度 wage inflation ∈ [1.47%, 5.01%]
low, high = 0.0147, 0.0501
violations = sum(1 for r in annual_wage_inflation if r < low or r > high)
n = len(annual_wage_inflation)
rate = violations / n * 100 if n else 0.0
print(f"\n=== 约束不满足率 (年度) ===")
print(f"Constraint range: [{low*100:.2f}%, {high*100:.2f}%]")
print(f"Violation rate: {violations}/{n} ({rate:.2f}%)")
